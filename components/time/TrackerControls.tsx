...
export default function TrackerControls({ state, onStart, onPause, onResume, onStop }: Props) {
  ...
  return <div className="flex flex-col gap-3 min-[380px]:flex-row">{state === "idle" ? <button type="button" onClick={onStart} className={primary}>{t("start")}</button> : null}{state === "running" ? <button type="button" onClick={onPause} className={secondary}>{t("pause")}</button> : null}{state === "paused" ? <button type="button" onClick={onResume} className={primary}>{t("resume")}</button> : null}{state !== "idle" ? <button type="button" onClick={onStop} className={secondary}>{t("stop")}</button> : null}</div>;
}
