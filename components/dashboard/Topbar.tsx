...
export default function Topbar({ onOpenMenu, mobileMenuOpen }: { onOpenMenu: () => void; mobileMenuOpen: boolean }) {
  return (
    <header className="sticky top-0 z-30 flex min-h-16 items-center gap-3 border-b border-white/10 bg-[#111827]/95 px-4 backdrop-blur sm:px-6 lg:px-8">
      <button type="button" onClick={onOpenMenu} aria-label="Open navigation" aria-expanded={mobileMenuOpen} aria-controls="mobile-dashboard-navigation" className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-[#9CA3AF] hover:bg-white/5 hover:text-[#E5E7EB] focus-visible:outline-2 focus-visible:outline-[#22C55E] lg:hidden"><span aria-hidden="true" className="text-xl">☰</span></button>
