  ...
  return (
    <details className="group relative">
      <summary aria-label="Open user menu" className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-lg px-1 focus-visible:outline-2 focus-visible:outline-[#22C55E] [&::-webkit-details-marker]:hidden">
        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#22C55E] text-sm font-bold text-[#07110B]">{currentUser.initials}</span>
        <span className="hidden text-sm font-medium text-[#E5E7EB] sm:block">{currentUser.firstName}</span>
      ...
      <div className="absolute right-0 top-12 z-40 w-52 rounded-xl border border-white/10 bg-[#161A34] p-2 shadow-2xl">
        <div className="border-b border-white/10 px-3 py-2"><p className="text-sm font-semibold text-[#E5E7EB]">{currentUser.name}</p><p className="text-xs text-[#9CA3AF]">{currentUser.role}</p></div>
